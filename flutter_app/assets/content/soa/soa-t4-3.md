...
> KMS는 키 자료를 하드웨어 보안 모듈(HSM) 기반 환경 안에서만 사용하도록 설계해 키 자료 노출 경로를 서비스 경계로 제한합니다.
...
> 식별자를 고정하고 키 자료만 새로 만들면 KMS가 내부적으로 세대를 추적해 과거 데이터는 과거 자료로, 신규 데이터는 새 자료로 처리해 애플리케이션 변경 없이 교체가 완성됩니다.
...
2. **"키를 교체하면 과거 데이터를 다시 암호화해야 한다."** → 아닙니다. 자동 교체는 **키 자료만** 바꾸고 키 ID/ARN/정책은 유지합니다. 과거 데이터는 과거 키 자료로 자동 복호화되어 재암호화가 필요 없습니다.
   *(원리: §2 — 키 식별자를 고정하고 키 자료만 교체하므로 KMS가 세대를 내부적으로 추적해 재암호화 없이 복호화가 이뤄진다.)*
...
6. **"CloudFront 인증서를 서비스 리전(예: 서울)에서 발급하면 된다."** → CloudFront 인증서는 반드시 **us-east-1**에서 발급해야 연결됩니다.
   *(원리: §7 — ACM 인증서 소유권이 발급 리전에 귀속되므로, CloudFront는 us-east-1 인증서만 연결 목록에 표시한다.)*
...
SSE-KMS 기본 구성에서는 객체 1개를 쓰거나 읽을 때마다 KMS API가 호출되어 데이터 키를 발급·복원합니다. 객체 수가 많을수록 KMS 호출 횟수가 선형으로 증가합니다. 버킷 키를 활성화하면 버킷 수준의 데이터 키를 재사용해 개별 객체마다 KMS를 호출하는 대신 훨씬 적은 횟수로 처리하므로, 대규모 워크로드에서 KMS 요청 수를 크게 줄일 수 있습니다.
</details>
