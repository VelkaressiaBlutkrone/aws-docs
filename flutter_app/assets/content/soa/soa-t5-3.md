...
> DNS 조회 요청이 어느 네임서버로 전달될지는 VPC의 DNS 리졸버가 결정합니다. 퍼블릭 호스팅 영역은 인터넷 전체에 공개된 Route 53 권한 네임서버로 쿼리가 라우팅되지만, 프라이빗 호스팅 영역은 연결된 VPC의 DNS 리졸버(169.254.169.253)에서만 응답합니다.
...
> 🧠 원리: 왜 CNAME은 Zone Apex(루트 도메인)에 사용할 수 없을까요?
> DNS 표준에 따르면 CNAME이 있는 이름에는 다른 어떤 레코드도 함께 존재할 수 없습니다. 그런데 루트 도메인에는 DNS 위임에 필수적인 SOA·NS 레코드가 반드시 있어야 하므로, 동시에 CNAME을 두면 충돌이 발생합니다.
> Route 53의 Alias는 DNS 응답 시 실제 AWS 리소스 IP를 직접 반환하는 방식이라 CNAME처럼 다른 이름으로 리다이렉트하지 않아 SOA·NS와 공존할 수 있습니다.
> 이 구조적 차이가 "루트 도메인에는 Alias, 서브도메인에는 CNAME도 가능"이라는 운영 규칙의 근거입니다.
...
> 🧠 원리: 왜 Latency 정책과 Geolocation 정책은 서로 다른 기준으로 트래픽을 분배할까요?
> Latency 정책은 Route 53이 각 리전과 사용자 간의 실측 네트워크 왕복 시간을 기반으로 동작합니다. 물리적으로 가깝더라도 인터넷 경로에 따라 지연이 달라질 수 있어, 지리적 거리와 실제 응답 속도가 항상 일치하지 않습니다.
> Geolocation 정책은 요청 IP를 GeoIP 데이터베이스로 조회해 국가·대륙을 판별합니다. 네트워크 지연과 무관하게 특정 국가 사용자를 지정 리전으로만 보낼 수 있어, 법적 규제·콘텐츠 현지화처럼 위치 기반 제어가 목적일 때 적합합니다.
> 두 정책은 목적이 달라 혼용해서는 안 됩니다. "빠른 응답"이 목표면 Latency, "위치 기반 라우팅"이 목표면 Geolocation입니다.
...
3. **"CloudFront용 ACM 인증서를 서울 리전에서 발급했다."** → CloudFront는 **us-east-1** 의 ACM 인증서만 사용합니다. 다른 리전 인증서는 연결할 수 없습니다(ALB용 인증서는 ALB와 같은 리전).
   *(원리: §6 — CloudFront는 us-east-1 외 리전 인증서를 연결할 수 없다.)*
...
6. **"OAI가 현재 권장 방식이다."** → OAI(Origin Access Identity)는 구형입니다. 현재 권장은 **OAC(Origin Access Control)** 입니다.
   *(원리: §6 — OAC는 서비스 주체 기반 정책으로 OAI보다 세밀한 접근 제어가 가능하다.)*
...
enableDnsSupport를 끄면 VPC 내 인스턴스는 AWS 제공 DNS 리졸버(169.254.169.253)를 사용하지 않습니다. 프라이빗 호스팅 영역의 레코드는 이 리졸버를 통해서만 응답되므로, 리졸버가 비활성화된 상태에서는 쿼리 자체가 Route 53 권한 네임서버에 도달하지 못해 이름 해석이 실패합니다. 퍼블릭 호스팅 영역은 인터넷 네임서버로 조회되므로 영향이 없습니다.
</details>
