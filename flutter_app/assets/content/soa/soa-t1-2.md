...
> 서비스마다 규정 준수·감사 요건이 달라 AWS가 일률적인 기간을 강제하기 어려운 측면도 있습니다.
> 그 결과 "로그가 사라질 위험"보다 "로그가 누적될 위험"을 기본 동작으로 선택했고, 운영자가 그룹마다 보존 기간을 명시해 의도적으로 제한하는 방식을 채택했습니다.
...
> Logs Insights는 로그를 영구 인덱스로 미리 가공해두지 않고, 쿼리가 실행될 때 선택한 시간 범위와 로그 그룹의 실제 데이터를 스캔합니다.
> 인덱스를 유지하지 않아 저장 비용이 절감되고 어떤 필드 조합이든 유연하게 쿼리할 수 있는 대신, 쿼리마다 실제로 읽어들인 데이터양이 비용을 결정합니다.
> 이 구조 때문에 시간 범위를 좁히거나 로그 그룹 수를 줄이면 같은 쿼리도 비용이 낮아집니다.
...
> 하이퍼바이저는 가상 머신에 할당된 CPU 사이클·네트워크 패킷·디스크 I/O 같은 물리 자원 사용을 관찰할 수 있지만, 가상 머신 내부의 주소 공간과 파일시스템 상태에는 접근하지 않습니다.
> 메모리 사용률은 게스트 OS 커널이 프로세스 메모리를 할당·해제하면서 관리하는 값이고, 디스크 여유 공간은 게스트 OS 파일시스템이 계산하는 값이므로 하이퍼바이저는 이 정보를 볼 수 없습니다.
> 에이전트를 게스트 OS 안에서 실행해야 이 내부 값을 읽어 CloudWatch로 전송할 수 있고, 그 결과가 `CWAgent` 네임스페이스에 나타납니다.
...
4. **"EC2를 만들면 메모리 사용률이 CloudWatch에 바로 나온다."** → 메모리·디스크 여유 공간은 게스트 OS 내부 값이라 기본 지표에 없습니다. **CloudWatch 에이전트**를 설치·구성해야 `CWAgent` 네임스페이스로 수집됩니다.
   *(원리: §6 — 하이퍼바이저는 게스트 OS 내부 주소 공간에 접근할 수 없어, 에이전트가 OS 안에서 직접 값을 읽어 전송한다.)*
