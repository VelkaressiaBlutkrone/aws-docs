...
| **SecureString** | SecureString | Parameter Store의 파라미터 유형 중 KMS 키로 암호화된 값 — 평문 String과 달리 저장·전송 중 암호화를 보장한다 |
...
> 전통적인 SSH는 클라이언트가 서버의 열린 포트에 연결을 시작하지만, Session Manager는 반대로 인스턴스의 SSM Agent가 SSM 서비스 엔드포인트로 아웃바운드 연결을 유지하고, 사용자 요청이 오면 그 기존 채널을 역방향으로 활성화합니다.
...
1. **"SSM Agent만 깔면 Run Command가 된다."** → 에이전트뿐 아니라 **IAM 인스턴스 프로파일**(`AmazonSSMManagedInstanceCore`)과 **SSM 엔드포인트 도달성**이 모두 있어야 관리형 노드가 됩니다. 보통 누락된 것은 IAM 역할입니다.
   *(원리: §1 — SSM 연결은 네트워크·IAM·Agent 세 링크로 이어진 체인이며, 하나가 끊기면 결과는 동일하게 "연결 없음"으로 나타난다.)*
...
5. **"비밀은 무조건 Secrets Manager여야 한다."** → 자동 교체가 필요 없고 비용을 아끼려면 **Parameter Store SecureString**(KMS 암호화)으로 충분합니다. **자동 교체(rotation)**가 핵심일 때만 Secrets Manager가 정답입니다.
   *(원리: §7 — 자동 교체는 단순 저장과 달리 오케스트레이션을 요구하며, 이 복잡성을 서비스화한 것이 Secrets Manager의 핵심 차별점이다.)*
...
6. **"프라이빗 서브넷에선 SSM을 못 쓴다."** → 인터넷이 없어도 **VPC 인터페이스 엔드포인트(ssm·ssmmessages·ec2messages)**를 만들면 SSM(Run Command·Session Manager·Patch)을 정상 사용할 수 있습니다.
   *(원리: §1 — 엔드포인트 도달성이 충족되면 나머지 조건(Agent·IAM)과 독립적으로 네트워크 요건이 해결되어 프라이빗 서브넷에서도 SSM 체인이 완성된다.)*
