...
> 두 규칙 유형은 트리거 방식이 근본적으로 달라 하나의 모델로 묶으면 이벤트 없이 실행되는 스케줄 측에 이벤트 수신 로직을, 이벤트 기반 측에 타이머 로직을 불필요하게 추가해야 합니다.
> AWS는 두 유형을 명시적으로 분리해 운영자가 반응형(이벤트 패턴)과 주기적(스케줄) 중 의도에 맞는 유형을 선택하도록 합니다.
...
> 자동 복구의 대상은 호스트 하드웨어·인프라 장애이며, 인스턴스 자체의 EBS·설정 상태는 문제가 없습니다.
...
> ASG는 인스턴스를 교체 가능 단위로 다루며, 문제의 원인이 OS·앱 구성일 때 같은 인스턴스에서 복구를 시도하면 동일 장애가 재발할 수 있습니다.
...
> Lambda는 "이벤트 페이로드를 받아 코드를 실행하고 종료"하는 모델로 동작하며, 어떤 소스가 호출하든 이벤트 객체를 통해 동일하게 처리됩니다.
> EventBridge·Config·경보 등 다양한 AWS 서비스가 Lambda를 트리거로 지원하기 때문에, 운영자는 트리거 종류마다 별도 실행 환경을 준비하지 않고 함수 코드에만 집중할 수 있습니다.
...
4. **"비규정 리소스를 자동으로 고치려면 Lambda를 직접 짜야만 한다."** → 많은 경우 **Config 규칙 + SSM Automation 교정 런북**으로 코드 없이 자동 교정할 수 있습니다. 표준 런북으로 안 되는 커스텀 로직에만 Lambda를 씁니다.
   *(원리: §6 — Config는 평가 전담, SSM Automation 런북이 실행 담당이므로 표준 작업은 런북으로 충분하다)*
...
6. **"ASG는 인스턴스 내부 앱이 죽어도 교체한다."** → 기본 **EC2 상태 점검만**으로는 앱 장애를 못 잡습니다. 애플리케이션 수준 장애로 교체하려면 **ELB 상태 점검**을 ASG에 연결해야 합니다.
   *(원리: §4 — ASG는 연결된 상태 점검이 감지한 장애만 교체하므로, 앱 응답 감지엔 ELB 상태 점검을 연결해야 한다)*
