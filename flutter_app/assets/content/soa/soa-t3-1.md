...
> Export를 바꾸려면 먼저 그 값에 의존하는 모든 스택을 분리해야 합니다. 이 절차가 번거롭더라도, 의존 관계를 명시적으로 끊는 과정에서 운영자가 영향 범위를 확인하게 됩니다.
> 결과적으로 교차 스택은 독립적으로 배포·업데이트되지만 Export 잠금을 통해 의존 스택들의 런타임 일관성이 유지됩니다.
...
> 대규모 조직에서 가드레일·로깅 같은 기준선 구성을 신규 계정에도 일관되게 배포하려면 서비스 관리형 모드가 필요합니다.
...
6. **"리소스를 in-place로 수정하니 안전하다."** → 변경 세트에 **Replacement: True**가 뜨면 해당 리소스는 **삭제 후 새로 생성**됩니다(새 물리 ID·데이터 손실 가능). 적용 전에 반드시 교체 여부를 확인해야 합니다.
   *(원리: §3 — Replacement: True는 리소스 삭제·재생성이므로 실행 전 반드시 확인한다.)*
