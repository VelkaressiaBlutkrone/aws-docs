...
| **가용 영역** | Availability Zone (AZ) | 리전 내 독립된 전원·냉각·네트워크를 갖춘 하나 이상의 데이터센터 — 마치 건물 동(棟)처럼 하나가 장애여도 다른 동은 정상 운영 |
...
> 복구 작업은 새 인스턴스를 생성하는 것이 아니라 기존 인스턴스의 가상 하드웨어 매핑을 정상 호스트로 재연결하는 방식으로 동작합니다.
> 인스턴스 ID·프라이빗 IP·Elastic IP는 가상 네트워크 레이어에서 인스턴스 객체에 귀속된 식별자이므로, 호스트가 바뀌어도 해당 객체가 유지되는 한 식별자도 그대로 이어집니다.
...
4. **"자동 복구는 새 인스턴스를 만든다."** → 복구는 **같은 인스턴스를 새 정상 호스트로 이전**하는 것입니다. 인스턴스 ID·프라이빗 IP·EIP가 보존됩니다(인메모리·인스턴스 스토어 데이터는 유실될 수 있음). 새 인스턴스 생성·교체는 Auto Scaling의 역할입니다.
   *(원리: §4 — 복구는 가상 하드웨어 매핑을 정상 호스트로 재연결하는 방식이어서, 인스턴스 객체가 유지되는 한 ID·IP 등 식별자도 이어진다.)*
