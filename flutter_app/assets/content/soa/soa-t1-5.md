...
> 🧠 원리: 왜 T 계열은 크레딧이 소진되면 standard 모드에서 성능이 기준선에 고정될까요?
> T 계열은 기준선 미만으로 동작하는 시간 동안 크레딧을 적립하고, 기준선 초과 동작 시 그 크레딧을 소비하는 방식으로 순간적인 높은 CPU를 허용합니다.
> standard 모드에서는 크레딧 잔량이 0이 되면 기준선을 넘는 처리를 더 이상 할 수 없어 CPU 사용량이 기준선에 묶이고, unlimited 모드에서는 버스트가 유지되지만 초과분에 추가 요금이 발생합니다.
> 이 구조 때문에 평소 한가하다가 가끔 높은 CPU가 필요한 워크로드에는 적합하지만, 지속적으로 높은 CPU가 필요한 워크로드에서는 어느 모드든 M/C 계열이 더 적합합니다.
...
> gp2는 사용하지 않은 IOPS 여유분을 버스트 버킷에 I/O 크레딧으로 쌓아두고, 기준선(3 IOPS/GB)을 초과하는 요청이 올 때 그 크레딧을 소모하는 방식으로 동작합니다.
> 버킷이 가득 찬 상태에서 순간 부하를 처리할 수 있지만, 크레딧 소모 속도가 누적 속도를 지속적으로 앞서면 버킷이 비워지고 기준선 IOPS로 제한됩니다.
...
1. **"CPU가 부족하면 무조건 인스턴스를 키운다."** → 먼저 **패밀리**를 확인합니다. 메모리 부족인데 컴퓨팅 최적화(C)를 키우면 낭비입니다. 또 만성 고CPU인데 **T 계열**을 쓰고 있다면 크레딧 소진으로 throttling되는 것이니, M/C 계열 전환이 정답일 수 있습니다.
   *(원리: §1 — standard 모드에서 크레딧 소진 시 기준선 제한, unlimited 모드에서도 추가 과금 — 지속 고부하엔 어느 모드든 T 계열보다 M/C 계열이 적합하다.)*
...
2. **"gp2에서 용량을 늘리지 않고 IOPS만 올린다."** → gp2는 **IOPS가 용량에 비례(3 IOPS/GB)**합니다. IOPS를 용량과 독립적으로 올리려면 **gp3로 전환**(베이스라인 3,000 IOPS, 추가 프로비저닝 가능)하거나 io1/io2를 씁니다.
   *(원리: §2 — gp2 버스트 버킷은 I/O 크레딧이 소모되면 기준선 IOPS로 제한되므로 지속 IOPS 확보에 gp3 전환이 필요하다.)*
...
3. **"작은 gp2 볼륨인데 항상 3,000 IOPS가 나온다."** → 그건 **버스트**입니다. 버스트 크레딧이 소진되면 baseline(용량×3 IOPS)으로 떨어집니다. 지속적으로 높은 IOPS가 필요하면 gp3/io2로 가야 합니다.
   *(원리: §2 — 버스트 버킷의 I/O 크레딧 소모 속도가 누적 속도를 지속 초과하면 버킷이 비워지고 기준선으로 제한된다.)*
