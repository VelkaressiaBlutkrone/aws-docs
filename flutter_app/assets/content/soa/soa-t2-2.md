...
| 용어 | 영문 | 한 줄 풀이 |
|---|---|---|
| **이중화** | Redundancy | 동일 리소스를 복수로 배치해 한 곳이 장애 나도 서비스가 이어지게 하는 설계 원칙 |
...
> 동시에 AZ 간에는 저지연 전용 링크를 유지해, 같은 리전 내에서는 AZ를 건너는 복제나 통신이 실용적인 속도로 이뤄질 수 있습니다.
> 이 "물리 분리 + 고속 연결" 조합이 단일 리전 안에서 장애 격리와 이중화를 동시에 구현하는 근거입니다.
...
> 애플리케이션은 IP 주소가 아닌 RDS 엔드포인트의 DNS 이름으로 연결합니다. 스탠바이가 프라이머리로 승격될 때 AWS는 그 DNS 이름이 가리키는 IP를 새 프라이머리로 갱신합니다.
> 애플리케이션이 연결 풀을 재설정하면 동일한 DNS 이름으로 새 인스턴스에 자동 연결되어, 인프라 변경이 코드 수정 없이 흡수됩니다.
> 이 DNS 간접 계층이 있기 때문에 장애 조치가 애플리케이션에 투명하게 처리됩니다.
...
> 이 잠재적 데이터 손실 위험 때문에 복제본 승격은 운영자가 상황을 판단한 뒤 수동으로 수행하도록 설계되어 있습니다.
...
> ELB는 대상 그룹의 헬스 체크 결과를 지속적으로 수집하며, 요청을 분배할 때 정상 상태인 대상에만 전달합니다.
> 한 AZ의 대상이 전부 헬스 체크에 실패하면 ELB는 그 AZ로의 트래픽 분배를 중단하고 나머지 AZ의 정상 대상에만 보내는 방식으로 자동 조정합니다.
> 이 동작이 ELB를 단순한 트래픽 분산기가 아니라 AZ 수준의 장애 격리 장치로 기능하게 합니다.
...
> 계층별 독립 이중화는 각 층의 장애 반경을 해당 계층으로 제한해 전체 연쇄 장애를 방지하는 근거입니다.
...
1. **"RDS Multi-AZ가 읽기 성능도 높여준다."** → 틀립니다. 단일 스탠바이는 읽기를 처리하지 않습니다(가용성 전용). 읽기 확장은 읽기 전용 복제본을 추가합니다.
   *(원리: §2 — DNS 간접 계층을 통해 장애 조치를 투명하게 처리하는 것이 Multi-AZ의 핵심 설계이고, 스탠바이는 읽기 서비스가 아니라 전환 대기 용도이다.)*

2. **"읽기 복제본은 자동으로 장애 조치된다."** → 읽기 복제본은 자동 장애 조치가 없습니다. **수동 승격(promote)**이 필요합니다. 자동 장애 조치는 Multi-AZ의 기능입니다.
   *(원리: §3 — 비동기 복제로 인한 잠재적 데이터 손실 때문에 복제본 승격은 운영자가 상황을 판단한 뒤 수동으로 수행하도록 설계되어 있다.)*
...
7. **"단일 AZ ASG도 고가용성이다."** → 단일 AZ ASG는 그 AZ 장애 시 전멸합니다. 최소 2개 AZ에 분산해야 합니다.
   *(원리: §7 — 계층별 독립 이중화는 각 층의 장애 반경을 해당 계층으로 제한하며, 단일 AZ는 컴퓨트 계층 전체가 하나의 장애 반경 안에 묶이는 구성이다.)*
