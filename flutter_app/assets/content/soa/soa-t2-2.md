...
> 장애 조치가 투명하게 이뤄지려면 세 가지가 맞물려야 합니다: ① 엔드포인트 이름 불변(AWS가 보장), ② AWS의 DNS 레코드 갱신(스탠바이 승격 시 IP 전환), ③ 앱의 연결 풀 재시도(기존 TCP 연결을 끊고 재연결).
> 앱이 연결 풀을 재시도할 때 OS 또는 JVM의 DNS 캐시가 갱신된 IP를 반환해 새 프라이머리에 연결되며, 이 과정에서 엔드포인트 문자열은 전혀 바뀌지 않습니다.
