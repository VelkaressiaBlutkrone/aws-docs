...
| **상태 추적 방화벽** | Stateful Firewall | 연결 상태를 기억해 응답 트래픽을 자동 허용하는 방식 — 문열면 답장도 통과 |
| **무상태 방화벽** | Stateless Firewall | 요청·응답을 별개로 평가해 각 방향 규칙이 모두 있어야 통과하는 방식 |
...
> SG는 연결 상태를 추적해 허용 목록에 없는 패킷을 응답도 없이 조용히 폐기하는데, 이 동작 자체가 네트워크 스택에서 REJECT 이벤트가 아니라 단순 드롭으로 처리됩니다.
...
2. **"연결이 안 되면 무조건 Flow Log에 REJECT가 찍힌다."** → **보안 그룹 거부는 REJECT로 잘 드러나지 않습니다**(SG는 stateful이라 미허용 인바운드를 조용히 버림). REJECT가 없는데 연결이 안 되면 **SG 인바운드**를 먼저 의심해야 합니다. NACL 거부라면 REJECT가 찍힙니다.
   *(원리: §3 — SG는 패킷을 조용히 드롭해 REJECT 이벤트 없이 차단하고, NACL은 명시적 거부로 REJECT를 남긴다.)*
