...
| 용어 | 영문 | 한 줄 풀이 |
|---|---|---|
| **비동기 복제** | Asynchronous Replication | 소스에서 쓰기 확인 후 보조 사이트로 별도 전송 — 복제 지연만큼 RPO가 0보다 클 수 있음 |
| **버전 관리** | Versioning | S3에서 동일 키의 객체를 여러 버전으로 보관하는 기능. CRR·SRR의 전제 조건 |
...
> 🧠 원리: 왜 4가지 DR 전략은 비용과 복구속도가 반비례하는 스펙트럼을 이룰까요?
> 각 전략의 핵심 차이는 재해 발생 전에 보조 리전에 얼마나 많은 인프라를 상시 가동 상태로 유지하는가에 있습니다.
> Backup & Restore는 데이터만 보관하므로 재해 시 인프라 전체를 처음부터 배포해야 하고, Multi-Site는 이미 모든 용량이 여러 리전에서 동작 중이라 별도 전환 단계가 없습니다.
> 미리 준비한 범위가 클수록 복구에 걸리는 시간이 짧아지지만, 그만큼 상시 운영 비용도 증가하므로 RTO·RPO 목표를 충족하는 최소 전략이 합리적 선택이 됩니다.
...
> 앱 서버는 AMI·시작 템플릿이 있으면 재해 시 수십 분 안에 새로 기동할 수 있지만, DB의 축적된 데이터는 사전 복제 없이는 짧은 시간에 보조 리전에서 따라잡을 방법이 없습니다.
> 따라서 앱 서버는 꺼두고 DB 복제만 상시 유지해도 RTO를 수십 분 수준으로 유지할 수 있으며, 앱 서버까지 상시 가동하는 Warm Standby보다 운영 비용을 낮출 수 있습니다.
> 이 비대칭 구성이 Pilot Light의 비용 절감 논리이며, DB와 앱 서버의 복구 가능성 차이에서 비롯됩니다.
...
> 자동으로 승격하면 그 미반영 데이터는 영구 손실되고, 이후 원본 Primary가 복구됐을 때 두 인스턴스 사이에 데이터 불일치가 발생할 수 있습니다.
> 운영자가 손실 범위를 확인하고 수동으로 승격을 결정하는 절차가 필요한 이유가 여기에 있으며, 이 과정이 RTO에 포함됩니다.
...
> 훈련은 설계와 현실의 차이를 발견하는 유일한 수단이며, 측정 없이는 RTO·RPO 목표 달성 여부를 보증할 수 없습니다.
...
1. **"RTO/RPO가 엄격하면 무조건 Active-Active."** → Active-Active가 가장 비쌉니다. RTO 수 분 허용 + 비용 제약이면 Warm Standby가 더 적합합니다.
   *(원리: §2 — 각 전략은 미리 가동 중인 인프라 범위로 비용과 복구속도가 결정되므로, RTO·RPO를 충족하는 최소 비용 전략을 선택해야 한다.)*
...
4. **"S3 복제를 켜면 기존 객체도 자동 복제된다."** → 복제는 활성화 **이후 추가/변경된 객체**에 적용됩니다. 기존 객체는 **S3 배치 복제(Batch Replication)**로 따로 복제해야 합니다.
   *(원리: §4 본문 — CRR·SRR은 이벤트 기반으로 동작해 활성화 이전 객체는 이벤트 대상에 포함되지 않는다.)*

5. **"RPO=0이면 Backup & Restore로 된다."** → Backup & Restore는 RPO가 마지막 백업 이후(수 시간)입니다. RPO 0에는 동기 복제 또는 Active-Active가 필요합니다.
   *(원리: §1 — RPO는 복제 방식이 좌우하며, 백업 주기만큼 반드시 손실이 발생해 RPO를 0으로 만들 수 없다.)*
...
7. **"Route 53 장애 조치는 즉각적이다."** → DNS TTL만큼 전파 지연이 있습니다. 짧은 TTL이나 Global Accelerator로 완화합니다.
   *(원리: §6 본문 — 헬스 체크 기반 장애 감지 후에도 DNS TTL 동안 클라이언트가 이전 IP를 캐시해 트래픽 전환이 지연된다.)*
