...
> 비용 최적화·내결함성·성능 점검은 계정 전반의 리소스 구성을 지속 분석해야 하므로, AWS 쪽 연산 비용과 유지 비용이 발생합니다.
> 기본 플랜을 사용하는 계정은 수가 매우 많아, 모든 점검을 무료로 제공하면 비용 구조상 지속하기 어렵습니다.
> 그래서 즉각적인 보안 위험(퍼블릭 S3·루트 MFA 미설정)과 서비스 한도 초과 경고처럼 운영 연속성에 직결된 일부 항목은 기본 플랜에도 열어두고, 나머지는 유료 지원 플랜과 묶는 방식을 취합니다.
...
7. **"Trusted Advisor 모든 점검이 무료다."** → 전체 점검 항목은 **Business/Enterprise Support 플랜**에서 제공됩니다. 기본 플랜은 일부만 열립니다.
   *(원리: §5 — 비용·성능·내결함성 점검은 지속 분석 비용이 발생하므로 유료 플랜과 묶이고, 보안·서비스 한도 일부만 기본 제공된다.)*
...
Config 자동 교정은 실행 흐름을 SSM Automation에 위임합니다. SSM Automation 런북은 여러 단계(검증 → 교정 → 알림)를 순서대로 처리하고, 실패 시 롤백·대기·재시도 로직을 런북 안에 정의할 수 있습니다. Lambda를 직접 호출하면 이 단계 제어와 실행 이력 추적이 모두 Lambda 코드 안에 들어가야 해 관리가 복잡해집니다. SSM Automation은 실행 이력을 콘솔에서 확인하고 AWS-managed 런북을 그대로 재사용할 수 있어, 별도 코드 없이 표준 교정 흐름을 구축할 수 있습니다.
</details>
