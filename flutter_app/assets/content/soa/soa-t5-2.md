...
| 용어 | 영문 | 한 줄 풀이 |
|---|---|---|
| **CIDR** | Classless Inter-Domain Routing | IP 주소 범위를 `/`(슬래시) 표기로 나타내는 방식 — 예: `10.0.0.0/16` |
| **IPsec** | IP Security | IP 패킷을 암호화하고 인증하는 프로토콜 묶음 — VPN 터널의 표준 암호화 계층 |
| **BGP** | Border Gateway Protocol | 라우터 간 경로 정보를 자동으로 교환하는 동적 라우팅 프로토콜 |
| **ENI** | Elastic Network Interface | 가상 네트워크 카드 — 서브넷 안에 배치되어 사설 IP를 갖는 AWS 네트워크 인터페이스 |
| **ISP** | Internet Service Provider | 인터넷 회선을 제공하는 통신 사업자 — DX는 ISP 경로를 우회해 전용 회선을 씀 |
| **prefix list** | Prefix List | AWS가 관리하는 IP 주소 범위 목록 — 게이트웨이 엔드포인트가 라우팅 경로로 이 목록을 씀 |
| **프라이빗 DNS** | Private DNS | VPC 내부에서만 해석되는 DNS 설정 — 인터페이스 엔드포인트가 서비스 기본 도메인을 사설 IP로 해석할 때 사용 |
...
> 🧠 원리: 왜 피어링은 A-B, B-C가 있어도 A-C 통신을 허용하지 않을까요?
> VPC 피어링은 두 VPC 사이에만 가상 경로를 만들고, 한 쪽이 받은 경로 정보를 세 번째 VPC로 다시 전달하는 전이 전파 기능이 없습니다.
> 패킷이 B를 경유해 C로 가려면 B의 라우팅 테이블이 C 방향 경로를 가져야 하고, B가 그 경로를 A에도 광고해야 하는데 피어링 구조에서는 그 광고 단계가 존재하지 않습니다.
> 운영상 피어링 수가 적을 때는 직접 피어링 추가로 해결하고, VPC 수가 늘어 메시가 복잡해지면 전이 라우팅이 내장된 Transit Gateway 전환이 현실적입니다.
...
> 🧠 원리: 왜 VPC 수가 늘어날수록 피어링 메시보다 TGW가 운영 부담이 낮아질까요?
> 피어링은 통신이 필요한 VPC 쌍마다 별도 연결을 만들어야 해 n(n-1)/2 개의 경로와 라우팅 테이블 항목이 각 VPC에 생깁니다.
> TGW는 VPC마다 허브에 어태치먼트 하나만 추가하면 되고, 경로 관리는 TGW 라우팅 테이블 한 곳에서 이루어지므로 VPC가 늘어도 각 VPC에서 관리할 경로 수는 크게 증가하지 않습니다.
> 온프레미스 연결(VPN·DX)도 TGW 한 곳에 붙이면 모든 어태치먼트에서 공유할 수 있어, 각 VPC마다 별도 VGW를 두는 방식 대비 구성 지점이 줄어듭니다.
...
> 🧠 원리: 왜 Site-to-Site VPN은 터널을 반드시 2개 만들까요?
> 두 터널은 AWS 측의 서로 다른 물리 엔드포인트에 종단되어, 한 엔드포인트의 유지보수나 장애가 다른 터널에 영향을 주지 않습니다.
> CGW에서 한 터널만 활성화하면 그 터널이 끊길 때 새 터널을 수동으로 올려야 하지만, 두 터널을 모두 상시 활성화하면 장애가 생긴 터널의 트래픽이 살아 있는 터널로 자동 전환됩니다.
> BGP 동적 라우팅을 쓰면 경로 재수렴도 자동으로 이루어져, 단일 터널 장애에 대한 중단 시간을 최소화할 수 있습니다.
...
> 🧠 원리: 왜 전용 물리 회선인 DX에 기본 암호화가 없을까요?
> 전용 회선의 '전용'은 전기 신호가 지나가는 물리 경로를 다른 고객과 공유하지 않는다는 의미이며, 이는 도청 경로를 줄이는 물리적 격리이지 데이터를 암호화하지는 않습니다.
> 암호화는 별도의 프로토콜 계층(IPsec 등)이 처리하는 영역이므로, 전용선 위에 의도적으로 얹지 않으면 평문 상태로 전송됩니다.
> 규정상 전송 암호화가 요구될 때는 DX 위에 IPsec VPN 터널을 추가(DX over VPN)해 논리적 암호화 채널을 구성합니다.
...
> 🧠 원리: 왜 S3·DynamoDB는 Gateway 방식으로 접근하고 다른 서비스는 ENI 방식(Interface)을 쓸까요?
> S3와 DynamoDB는 AWS가 해당 서비스의 퍼블릭 IP 범위를 prefix list라는 관리형 목록으로 유지하고, 라우팅 테이블에 "이 IP 범위는 VPC 외부 인터넷이 아닌 vpce 게이트웨이로 보내라"는 경로를 추가하는 방식이 동작합니다.
> 반면 대부분의 다른 AWS 서비스는 퍼블릭 IP 범위를 prefix list로 분리 관리하지 않거나 서비스별 엔드포인트 주소가 동적이어서, 라우팅 경로 하나로 처리하기 어렵습니다.
> Interface 엔드포인트는 서브넷에 ENI와 사설 IP를 직접 만들어 서비스 도메인이 그 사설 IP로 해석되게 하므로, IP 범위 관리 방식에 관계없이 어떤 서비스든 프라이빗하게 접근할 수 있습니다.
...
**Q5 (원리).** 왜 Gateway 엔드포인트로 접근한 S3 트래픽에는 보안 그룹을 적용할 수 없는가요?

<details><summary>정답 보기</summary>

Gateway 엔드포인트는 서브넷 라우팅 테이블에 경로를 추가하는 방식으로 동작해, 트래픽이 특정 네트워크 인터페이스(ENI)를 통과하지 않습니다. 보안 그룹은 ENI에 연결되어 ENI를 드나드는 트래픽을 제어하는 구조이므로, ENI가 없는 Gateway 엔드포인트에는 보안 그룹을 붙일 수 없습니다. 접근 제어가 필요하면 엔드포인트 정책으로 어떤 IAM 주체가 어떤 버킷·테이블에 접근할 수 있는지를 제한합니다.
</details>
