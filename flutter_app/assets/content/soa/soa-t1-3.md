...
> S3 `GetObject`·`PutObject`, Lambda `Invoke` 같은 데이터 평면 작업은 애플리케이션 로직이 실행될 때마다 발생해, 규모가 큰 계정에서는 초당 수천 건이 쌓이는 고볼륨 이벤트입니다.
...
1. **"CloudTrail은 S3 객체 다운로드까지 기본으로 다 기록한다."** → S3 `GetObject` 같은 **데이터 이벤트는 기본 기록되지 않습니다.** 객체 수준 감사는 데이터 이벤트를 명시적으로 켜야 하며 고볼륨이라 추가 비용이 듭니다.
   *(원리: §1 — 데이터 이벤트는 초당 수천 건이 발생하는 고볼륨 특성으로 기본 기록에서 제외되며, 명시적 활성화로 범위를 지정해야 한다.)*
