...
> IAM을 글로벌 단일 상태로 유지하면 서울 리전의 EC2가 수임한 역할과 버지니아 리전의 Lambda가 수임한 역할의 권한이 항상 같은 정책 문서를 참조합니다.
...
> 여러 정책(아이덴티티 기반·리소스 기반·SCP·권한 경계)이 동시에 적용되는 환경에서 Deny가 Allow와 동등한 우선순위라면, 한 레이어의 Deny를 다른 레이어의 Allow로 덮어쓸 수 있어 가드레일이 무력화됩니다.
> Deny 최우선 규칙은 SCP나 권한 경계에 단 하나의 Deny만 있어도 어떤 아이덴티티 정책의 Allow도 무효화할 수 있게 해, 계층적 보안 통제가 의도대로 작동합니다.
> 기본 거부(암묵적 Deny)와 결합하면 "명시적으로 Allow된 것만 허용, 나머지는 모두 차단"이라는 최소 권한 원칙이 평가 로직 자체에 내재됩니다.
...
2. **"역할은 영구 권한이다."** → 역할 수임 시 STS가 발급하는 자격증명은 **임시**(기본 1시간, 설정에 따라 최대 12시간)입니다. 만료되면 재수임이 필요합니다.
   *(원리: §5 — 신뢰 정책과 권한 정책이 분리돼 있어 수임 가능 주체와 권한 범위를 독립적으로 변경할 수 있다.)*
...
**여러 정책 레이어(아이덴티티 기반·리소스 기반·SCP·권한 경계)가 동시에 존재하는 환경에서 가드레일의 효력을 보장하기 위해서입니다.** Deny가 Allow와 같은 우선순위라면 한 레이어의 Deny를 다른 레이어의 Allow로 덮어씌울 수 있어, SCP나 권한 경계로 설정한 제한이 무력화됩니다. Deny 최우선 규칙은 어느 레이어에서든 단 하나의 Deny만으로 모든 Allow를 무효화할 수 있게 해, 기본 거부(deny-by-default) 원칙이 평가 로직에 내재됩니다.
</details>
