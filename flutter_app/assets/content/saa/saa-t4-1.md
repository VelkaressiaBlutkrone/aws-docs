...
| **버스트 크레딧** | Burst Credit (I/O Credit) | 평소 낮은 부하에서 크레딧을 쌓아두었다가 순간 부하 시 소진하는 방식 — 크레딧이 소진되면 성능이 기본 수준으로 떨어짐 |
| **rightsizing** | Rightsizing | 실제 사용량에 맞게 자원 크기를 줄여 비용을 최적화하는 과정 |
...
> 이 분리 구조가 수명주기 정책과 Intelligent-Tiering이 작동하는 경제적 근거입니다.
...
> IA·Glacier 클래스는 저장 단가가 낮은 대신, 장기 보관을 전제한 인프라 비용 구조로 설계되어 있습니다.
> 최소 기간 이전에 삭제·전환이 일어나면 해당 클래스에서 기대한 사용 기간을 채우지 못했으므로, 남은 기간 요금을 청구해 단가 낮춤의 전제 조건을 유지합니다.
> 이 구조 때문에 30일 이전에 Standard-IA로 전환하면 절감이 아니라 오히려 비용이 늘어날 수 있습니다.
...
> 이 연동 구조는 데이터가 적더라도 성능을 위해 빈 용량에 비용을 지불하게 만들었으며, 과잉 프로비전의 주된 원인이었습니다.
...
> S3 Gateway Endpoint는 이 경로를 우회해 NAT Gateway 처리 요금을 없애는 대표적인 최적화 수단입니다.
...
1. **"Glacier는 무조건 저렴하다."** → 검색 요금이 GB당 과금되고 최소 보관 기간(90~180일)이 있습니다. 자주 꺼내는 데이터를 Glacier에 두면 Standard보다 비쌀 수 있습니다.
   *(원리: §1 — 저장 요금과 검색 요금은 반비례 관계이며, 접근 빈도가 높을수록 검색 요금이 총비용을 역전시킬 수 있다.)*

2. **"EC2를 Stop하면 EBS 요금도 안 낸다."** → EBS는 인스턴스가 중지 상태여도 프로비전된 용량에 대해 계속 과금됩니다. 완전히 삭제(Terminate)해야 볼륨 요금이 멈춥니다.
   *(원리: §4 🧠 — 프로비전된 용량은 실제 사용량과 무관하게 비용이 발생하므로, 필요 없는 볼륨은 삭제해야 과금이 멈춘다.)*
...
3. **"수명주기 전환은 언제 해도 최소 보관 기간 요금이 없다."** → Standard-IA는 30일, Glacier 계열은 90일(Deep Archive 180일) 미만 보관 시 해당 기간 요금이 청구됩니다. 30일 이전에 Standard-IA로 전환하면 절감이 아닙니다.
   *(원리: §3 🧠 — IA·Glacier 클래스는 장기 보관 전제의 비용 구조이므로 최소 기간 미달 시 잔여 기간 요금이 청구된다.)*
...
5. **"NAT Gateway는 보안 도구라 비용 최적화와 무관하다."** → NAT Gateway는 **데이터 처리 요금(GB당)** 이 별도로 청구됩니다. S3·DynamoDB는 Gateway Endpoint로 우회하면 NAT 처리 비용이 발생하지 않습니다.
   *(원리: §5 🧠 — S3 Gateway Endpoint는 NAT Gateway 경로를 우회해 AZ 간 처리 요금 없이 S3에 접근하게 한다.)*
