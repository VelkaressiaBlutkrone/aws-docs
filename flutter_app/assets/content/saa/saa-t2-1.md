...
**순서 보장과 중복 제거에 필요한 추가 조율 비용 때문입니다.** Standard 큐는 메시지를 여러 서버에 분산 저장하고 어떤 서버에서든 꺼내도 되므로 수평 확장이 자유롭습니다. FIFO 큐는 동일 메시지 그룹(MessageGroupId) 내에서 순서를 보장하려면 특정 파티션에 메시지를 순서대로 배치하고, 소비자도 같은 파티션을 순서대로 처리해야 합니다. 중복 제거 ID(DeduplicationId)는 5분 창 내 동일 ID의 재요청을 추적·거부하는 상태를 유지해야 합니다. 이 두 제약이 분산 확장을 제한해, 기본 300 TPS(배치 시 3,000 TPS)라는 처리량 상한이 생깁니다.
</details>
