...
- [ ] **스토리지 클래스 선택** — 접근 빈도·복원 시간·비용을 기준으로 주요 클래스를 선택할 수 있다
...
- CLF에서 S3를 "객체 스토리지" 수준으로 봤다면, SAA는 **주요 S3 스토리지 클래스의 비용 구조·최소 보관 기간·복원 시간 차이**와 **접근 제어 4계층의 선택 기준**을 묻습니다.
...
### 2) 스토리지 클래스 — 접근 빈도 × 복원 시간 × 비용 {#storage-classes}
...
| **S3 Express One Zone** | 단일 AZ 초저지연·고성능 워크로드 | 99.95% | **1** | 없음 | 한 자릿수 밀리초 | 요청 요금 절감 |
...
| **S3 Glacier Deep Archive** | 연 1회 미만 초장기 보관 | 99.99% (복원 후) | 3 이상 | **180일** | **표준 12h·대량 48h** | GB당 과금 |

> **클래스 선택 공식** (시험 핵심):
> - 접근 패턴 모름 → **Intelligent-Tiering**
> - 가끔 접근·재생성 불가 → **Standard-IA**
> - 가끔 접근·재생성 가능 → **One Zone-IA** (비용 더 낮음)
> - 단일 AZ 안에서 초저지연 객체 접근 필요 → **S3 Express One Zone**
...
> AWS 공식 문서는 최소 보관 기간을 요금 규칙으로 명시합니다 — 기간 이전에 삭제·전환해도 해당 기간만큼의 요금이 청구됩니다.
> 이 규칙은 각 클래스가 특정 보관 주기를 전제로 설계되어 있음을 시사하며, 기간 도달 전에 객체를 제거하면 예상보다 비용이 높아질 수 있습니다.
> 따라서 보관 기간이 불확실하거나 짧을 가능성이 있는 데이터는 최소 보관 기간이 없는 클래스(Standard·Intelligent-Tiering)가 더 적합할 수 있습니다.

### 3) 수명 주기 정책 (S3 Lifecycle) {#s3-lifecycle}
...
### 6) 버전 관리와 복제 {#versioning-replication}
...
1. **"One Zone-IA나 S3 Express One Zone에 유실 불가 원본을 저장한다."** → 단일 AZ 장애 시 데이터 영구 유실 가능. One Zone 계열은 재생성 가능한 데이터나 단일 AZ 지연 최적화 워크로드에만 사용합니다.
   *(원리: §1 — 다중 AZ 중복 저장이 내구성의 물리적 근거이며, One Zone 계열은 이 안전망을 포기한 설계다.)*
...
3. **"Glacier에 저장하면 바로 꺼낼 수 있다."** → Glacier Flexible Retrieval과 Deep Archive는 아카이브 상태로 저장되며 **복원(Restore) 요청** 후에야 접근 가능합니다. 즉시 접근이 필요하면 **Glacier Instant Retrieval**을 사용합니다.
   *(원리: §2 본문 — 복원 시간은 스토리지 클래스 선택의 핵심 기준 중 하나이며, 클래스마다 복원 시간이 다르다.)*
...
4. **"수명 주기 전환 후 언제든 삭제해도 된다."** → Standard-IA·One Zone-IA는 30일, Glacier 클래스는 90일(Deep Archive는 180일) 미만 보관 시 **최소 보관 기간에 해당하는 요금이 청구**됩니다.
   *(원리: §2 — 최소 보관 기간은 각 클래스가 특정 보관 주기를 전제로 설계되었음을 시사하는 요금 규칙이다.)*
...
6. **"복제 설정 후 기존 객체도 자동으로 복제된다."** → 복제는 설정 이후 생성·업데이트되는 **신규 객체**에만 적용됩니다. 기존 객체는 S3 Batch Replication을 별도 실행해야 합니다.
   *(원리: §6 본문 — 복제는 설정 이후 신규 객체에만 적용되며, 기존 객체에는 자동 소급되지 않는다.)*
