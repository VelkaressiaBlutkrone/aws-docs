...
> 이 원칙은 saa-t2-2의 Lambda 스테이트리스 설계(§1)와 동일한 맥락으로, 실행 단위(Lambda 함수 vs EC2 인스턴스)가 달라도 수평 확장과 교체 가능성을 확보하려면 상태를 실행 단위 밖으로 분리해야 한다는 공통 원리입니다.
...
1. **"RDS Multi-AZ는 읽기 성능도 높여준다."** → 틀렸습니다. 스탠바이 인스턴스는 읽기 트래픽을 처리하지 않습니다. 읽기 확장이 필요하면 읽기 복제본을 추가합니다.
   *(원리: §4 — 동기 복제로 데이터 유실을 막는 것이 목적이라 스탠바이는 페일오버 대기 전용이며 읽기를 처리하면 복제 일관성이 흔들린다.)*
...
**동기 복제의 일관성 보장 때문입니다.** 스탠바이는 Primary와 항상 동일한 데이터 상태를 유지하기 위해 모든 쓰기를 Primary와 동시에 받습니다. 만약 스탠바이가 읽기 트래픽도 처리한다면, 읽기 요청 처리 중 페일오버가 발생했을 때 스탠바이가 새 Primary로 승격되면서 기존 읽기 세션이 예기치 않게 끊어질 수 있습니다. 또한 읽기와 복제 트래픽이 동일 인스턴스에서 경합하면 복제 지연이 발생할 위험이 있어 "데이터 유실 없음" 보장을 흔들 수 있습니다. 읽기 확장이 필요하면 읽기 복제본(비동기 복제)을 별도로 추가해 목적을 분리하는 것이 AWS 권장 설계입니다.
</details>
