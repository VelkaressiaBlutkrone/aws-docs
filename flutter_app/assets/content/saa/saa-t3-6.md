...
| **장애 조치** | Failover | 주(Primary) 노드에 장애가 발생했을 때 보조(Replica) 노드를 자동으로 승격해 서비스를 유지하는 절차 |
| **샤딩** | Sharding | 데이터를 여러 노드에 분산 저장해 수평 확장하는 기법 — 각 조각을 샤드(shard)라 부름 |
...
> LSI는 기본 테이블과 동일한 파티션 키를 공유하고 처리량도 함께 사용하므로, 파티션 레이아웃이 테이블 생성 시점에 LSI 존재를 전제로 구성됩니다.
> 이미 데이터가 적재된 파티션 구조를 바꾸지 않고 새 인덱스를 끼워 넣는 것은 내부 스토리지 레이아웃 상 불가능하며, 그 제약이 API 규칙으로 노출된 것입니다.
> 반면 GSI는 기본 테이블과 별도 파티션을 사용하므로 기존 데이터를 점진적으로 채워 넣는 방식으로 언제든 추가할 수 있습니다.
...
> 강력한 일관성 읽기는 응답 시점 기준으로 가장 최신 쓰기가 반영된 값을 보장해야 하므로, 복사본 간 합의가 이뤄진 결과를 확인하는 추가 I/O가 발생합니다.
> 이 추가 처리 비용이 RCU 소비량 두 배로 이어지며, 그만큼 처리량 예산도 더 많이 필요합니다.
...
### 12) Redis vs Memcached 비교 (★ 단골 출제)
...
1. **"LSI를 테이블 생성 후 추가할 수 있다."** → 불가능합니다. LSI는 테이블 생성 시에만 정의할 수 있습니다. 나중에 새 접근 패턴이 필요하면 **GSI**를 추가해야 합니다.
   *(원리: §3 — LSI는 파티션 레이아웃 구성 시 전제되므로 사후 추가가 구조적으로 불가하며, GSI는 별도 파티션을 써서 언제든 추가 가능하다.)*

2. **"GSI에서 강력한 일관성 읽기를 쓸 수 있다."** → GSI는 최종 일관성 읽기만 지원합니다. 강력한 일관성이 필요하면 기본 테이블 또는 LSI를 사용해야 합니다.
   *(원리: §4 — 강력 일관성은 복사본 합의 확인 I/O를 필요로 하며, GSI는 이 경로를 지원하지 않는다.)*
