...
| **동기 복제** | Synchronous Replication | 주 DB에 쓰기가 완료되기 전에 복제본에도 확인을 받는 방식 — 데이터 손실이 없는 대신 지연이 추가됨 |
| **비동기 복제** | Asynchronous Replication | 주 DB가 쓰기를 완료한 뒤 복제본에 전달하는 방식 — 쓰기 성능을 유지하되 짧은 복제 지연이 생길 수 있음 |
...
> 스탠바이를 읽기에서 격리해 두면, 페일오버 시점에 스탠바이가 주 인스턴스와 완전히 동기화된 깨끗한 상태임을 보장할 수 있습니다.
> 공식 문서가 명시하지 않지만, 이 설계는 HA 전용이라는 단일 목적에 집중해 복잡성을 낮추는 방향으로 이어집니다.
...
> 따라서 Serverless v2는 부하가 드문드문 있거나 예측 불가한 워크로드에 비용 효율적인 선택이 됩니다.
...
2. **"Read Replica는 자동 페일오버를 제공한다."** → 아닙니다. Read Replica는 비동기 복제이며, 주 인스턴스 장애 시 자동으로 승격되지 않습니다. 수동으로 승격해야 독립 인스턴스가 됩니다.
   *(원리: §3 — 비동기 복제는 쓰기 성능을 위해 복제 지연을 허용하므로, 장애 시점에 복제본이 주 인스턴스와 완전히 동기화되지 않을 수 있어 자동 승격을 보장하지 않는다.)*
