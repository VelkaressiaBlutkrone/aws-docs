...
5. **"OU에 SCP를 붙이면 상위 OU의 SCP가 무시된다."** → SCP는 **누적 교집합**입니다. 루트→OU→계정 순서로 모든 SCP를 AND로 교집합한 결과가 유효 권한의 상한선입니다. 상위 SCP가 막은 권한은 하위에서 열 수 없습니다.
   *(원리: §1 — OU 트리의 상속 모델이 하위 계층에서 상위 차단을 복구하는 경로를 구조적으로 없애 중앙 가드레일을 보장한다.)*
...
**상위 계층에서 설정한 가드레일을 하위 계층이 우회하지 못하도록 하기 위해서입니다.** 만약 하위 OU SCP가 상위 루트 SCP보다 넓은 허용 범위를 지정할 수 있다면, 루트에 설정한 조직 전체 가드레일이 OU 단위로 무력화됩니다. 누적 교집합 모델은 상위 계층에서 차단된 권한이 하위 어느 레벨의 Allow로도 복구되지 않음을 보장해, 중앙에서 설정한 제어가 조직 전체에 일관되게 유지됩니다.
</details>
