...
| **목적별 특화 DB** | Purpose-Built Database | 특정 데이터 모델·접근 패턴에 최적화해 설계된 DB 서비스 — 범용 관계형 DB 대신 쓰면 비용과 성능을 함께 개선할 수 있음 |
...
> RDS는 AWS가 내부적으로 해당 조합의 물리 인프라를 미리 확보하는 방식으로 할인을 제공하므로, 약정한 조합과 실제 사용 조합이 일치해야 할인이 적용됩니다.
> 이 구조 때문에 워크로드가 변경되어 인스턴스 클래스를 바꾸거나 리전을 이동하면 기존 RI가 사용되지 않아 낭비가 생길 수 있습니다.
...
2. **"Aurora Serverless v2는 항상 무료에 가깝다."** → 최솟값 ACU를 0으로 설정하지 않으면 유휴 상태에서도 최솟값만큼 과금됩니다. 개발 환경에서는 자동 일시정지(Pause) 설정을 확인하세요.
   *(원리: §4 — Aurora Serverless v2는 공유 스토리지 위에서 컴퓨팅 용량을 동적으로 조정하므로 최솟값 이하로는 내려가지 않는다.)*
...
RDS RI는 인스턴스 클래스·엔진·리전·Multi-AZ 여부의 특정 조합에 대해 구매되며, 실제 사용 조합이 일치할 때만 할인이 적용됩니다. Compute Savings Plans는 EC2·Fargate·Lambda의 컴퓨팅 사용량 총액에 대한 약정이라 서비스·인스턴스 유형에 무관하게 적용되는 반면, RDS RI는 조합 단위 할인이기 때문에 조합이 달라지면 할인이 적용되지 않습니다. 이 차이로 인해 RDS 워크로드의 안정성이 검증된 뒤에 RI를 구매해야 낭비를 줄일 수 있습니다.
</details>
