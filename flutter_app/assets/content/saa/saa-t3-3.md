...
- [ ] **배치 전략 구분** — 클러스터·파티션·분산·정밀 시간의 차이와 제한을 설명하고, 시나리오에 맞게 고를 수 있다
...
> 현행 고성능 EC2 인스턴스는 대부분 **Nitro 하이퍼바이저** 기반입니다. Nitro는 전용 하드웨어에 가상화 기능을 오프로드해 호스트 CPU 오버헤드를 최소화하고, 거의 베어메탈 수준의 성능을 제공합니다.
...
### 2) 인스턴스 패밀리 비교 {#instance-families}

| 패밀리 | 코드 | vCPU : 메모리 비율 | 주요 용도 | 대표 유형 |
|---|---|---|---|---|
...
### 4) 구매 옵션 — 성능·가용성 관점 {#purchase-options}
...
### 5) 배치 전략 비교 {#placement-strategies}

배치 그룹(Placement Group)은 EC2 인스턴스의 물리적 배치를 제어해 성능 또는 가용성 목표를 달성하는 논리적 그룹입니다. 생성 자체는 무료입니다.

| 속성 | 클러스터(Cluster) | 파티션(Partition) | 분산(Spread) | 정밀 시간(Precision time) |
|---|---|---|---|---|
| **배치 방식** | 단일 AZ 내 밀집 배치 | 파티션(랙 집합)별 분리 | 인스턴스마다 별도 랙 | 고정밀 시간 소스 접근 가능한 하드웨어에 배치 |
| **AZ 범위** | 단일 AZ만 | 동일 리전 다중 AZ 가능 | 동일 리전 다중 AZ 가능 | 지원 하드웨어가 있는 AZ |
| **최대 인스턴스 수** | 제한 없음(용량 의존) | AZ당 최대 7파티션, 인스턴스 수 제한 없음 | AZ당 최대 **7개** | 용량·지원 하드웨어 의존 |
| **네트워크 성능** | 인스턴스 간 단일 플로우 최대 **10 Gbps** (ENA 기준) | 파티션 내 일반 성능 | 일반 성능 | 네트워크가 아니라 시간 동기화 최적화 |
| **장애 격리** | 낮음 (동일 랙 공유 가능) | 파티션 단위 (랙·전원 분리) | 최고 (인스턴스별 별도 랙) | 목적 아님 |
| **주요 워크로드** | HPC·MPI·저지연 클러스터 | Hadoop·Cassandra·Kafka | 소수 핵심 인스턴스 (ZooKeeper·마스터 노드) | 분산 DB·금융 시스템·이벤트 순서화 |
| **전용 호스트** | 불가 | 불가 | 불가 | 불가 |
| **Spot 중지/최대 절전** | 불가 | 불가 | 불가 | 불가 |
...
> 다중 AZ 분산이 필요하다면 저지연 목표를 포기하고 파티션 또는 분산 배치 그룹을 선택해야 합니다. 마이크로초 수준의 시간 동기화가 핵심이면 정밀 시간 배치 그룹이 별도 선택지입니다.

### 6) 향상된 네트워킹 — ENA와 EFA {#enhanced-networking}
...
- **분산 시스템의 마이크로초급 시간 동기화** → 정밀 시간(Precision time) 배치 그룹.
...
7. **"정밀 시간 배치 그룹은 또 다른 저지연 네트워크 옵션이다."** → 정밀 시간 배치 그룹은 네트워크 처리량이 아니라 고정밀 시간 소스 접근을 위한 배치 전략입니다. HPC 네트워크 지연은 클러스터 배치 그룹과 EFA/ENA로 풀고, 이벤트 순서화·정확한 타임스탬프가 핵심이면 정밀 시간 배치를 검토합니다.
   *(원리: §5 본문 — 정밀 시간 배치 그룹은 고정밀 시간 소스가 있는 하드웨어에 인스턴스를 배치해 마이크로초급 시간 동기화를 돕는 전략이다.)*
