...
| **단일 테넌트** | Single Tenancy | 물리 서버를 다른 고객과 공유하지 않고 한 계정만 독점 사용하는 격리 방식 |
...
> 각 세대는 Nitro 하이퍼바이저와 연결되는 전용 오프로드 칩(스토리지·네트워크·보안)이 함께 업그레이드됩니다.
> 오프로드 칩이 발전할수록 가상화 기능을 처리하는 호스트 CPU 부담이 줄어 고객 워크로드가 사용할 수 있는 실질 처리 능력이 늘어납니다.
> 같은 패밀리 내에서도 세대가 올라가면 단순 클럭 향상이 아닌 하드웨어 아키텍처 전반의 교체가 이뤄지는 이유입니다.
> 따라서 새 세대 인스턴스를 선택하는 것은 가격 대비 성능을 높이는 가장 직접적인 방법 중 하나입니다.
...
> 2분은 체크포인팅·큐 반환·그레이스풀 셧다운에 충분한 시간으로 결정된 값이며, 이 유예 덕분에 재시작 가능한 배치 워크로드에서 스팟을 안전하게 활용할 수 있습니다.
...
> AZ 간 통신은 반드시 AZ 경계 라우터와 물리 케이블을 거치므로, 아무리 고대역폭 링크라도 동일 랙·스위치 내 통신보다 지연이 커질 수밖에 없습니다.
...
4. **"EFA는 ENA 대신 표준 IP 통신을 하지 않는다."** → EFA(EFA with ENA)는 ENA 디바이스도 함께 제공하므로 표준 IP 통신이 가능합니다. OS-bypass는 EFA 디바이스를 통하는 별도 경로입니다.
   *(원리: §6 — SR-IOV 기반 ENA 기능이 EFA에 포함되므로, OS-bypass 경로와 표준 IP 경로가 동시에 존재하는 구조다.)*
