...
| **라우팅 테이블** | Route Table | 서브넷의 트래픽을 어느 게이트웨이·엔드포인트로 보낼지 정의하는 VPC 규칙 집합 |
...
> 각 VPC 쌍은 별도의 피어링 연결이 필요합니다. VPC가 N개이면 가능한 쌍의 수는 N×(N-1)/2개로, N이 작을 때는 증가폭이 크지 않지만 N이 커질수록 제곱에 가깝게 증가합니다.
> 예를 들어 VPC 5개에서는 연결이 10개이지만 VPC 20개에서는 190개가 됩니다. 연결마다 양쪽 라우팅 테이블을 관리해야 하므로 운영 부담도 함께 폭증합니다.
...
VPC 피어링은 완전한 연결을 위해 VPC 쌍마다 개별 연결이 필요하므로, N개 VPC의 완전 연결에 N×(N-1)/2개의 연결이 필요합니다. VPC 수가 늘수록 연결 수와 각 연결의 라우팅 테이블 관리 부담이 함께 증가합니다. Transit Gateway는 각 VPC가 허브에 연결하는 구조여서 연결 수가 N개로 유지되고 라우팅도 중앙에서 관리됩니다. 데이터 처리 요금이 추가되지만 대규모 환경에서는 운영 복잡도 절감이 이 비용을 상쇄할 수 있습니다.
</details>
