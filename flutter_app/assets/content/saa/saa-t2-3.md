...
**특정 인스턴스에 세션이 고정되면 그 인스턴스가 장애로 교체될 때 해당 세션이 소실되기 때문입니다.** ASG가 장애 인스턴스를 종료하고 새 인스턴스를 시작해도, 기존 쿠키가 이미 삭제된 인스턴스를 가리키고 있어 해당 사용자는 세션이 끊기는 경험을 합니다. ElastiCache 같은 외부 세션 저장소를 사용하면 세션 데이터가 인스턴스 수명 주기와 분리되므로, 인스턴스가 교체되어도 어떤 인스턴스든 세션을 읽어 처리할 수 있어 ASG 자가 복구가 사용자에게 투명하게 작동합니다.
</details>
