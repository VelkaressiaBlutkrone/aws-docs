...
> Nitro 기반 호스트는 전용 하드웨어 오프로드 칩을 통해 데이터가 네트워크를 떠나기 전(호스트 → EBS 스토리지 서버 전송 중)에 투명하게 암호화하므로, 인스턴스 내부 프로세스나 하이퍼바이저 레이어가 평문 데이터를 볼 수 없습니다.
...
> EFS는 파일 데이터를 AWS가 관리하는 분산 스토리지 클러스터에 저장하며, 클러스터 자체는 필요에 따라 내부적으로 노드를 추가합니다. 사용자가 마운트하는 파일시스템은 이 클러스터의 논리적 뷰이므로, 실제 물리 용량 할당은 사용자에게 노출되지 않습니다.
> EBS처럼 볼륨 크기를 미리 예약하지 않아도 되는 이유는, 파일이 추가될 때마다 클러스터 레이어가 스토리지를 자동으로 확보하기 때문입니다.
...
1. **"여러 EC2가 EBS를 공유하면 된다."** → EBS는 기본적으로 단일 인스턴스 연결입니다. 여러 인스턴스가 동시에 같은 파일에 접근해야 하면 EFS(Linux) 또는 FSx for Windows(Windows)를 사용합니다. io1·io2의 Multi-Attach는 존재하지만, 클러스터 파일시스템이 없으면 데이터 충돌이 발생할 수 있습니다.
   *(원리: §2 — EBS는 같은 AZ 내 단일 인스턴스와 연결하도록 설계되어 있어, 파일 수준 동시성 제어가 없다.)*
...
4. **"EFS는 Windows EC2에서도 사용할 수 있다."** → Amazon EFS는 Windows 기반 EC2를 공식 지원하지 않습니다. Windows 공유 파일시스템에는 FSx for Windows File Server(SMB·AD)를 사용합니다.
   *(원리: §6 — EFS는 NFSv4 프로토콜 기반으로 Linux 파일시스템 의미론을 전제하므로, Windows의 SMB 프로토콜 스택과 호환되지 않는다.)*
