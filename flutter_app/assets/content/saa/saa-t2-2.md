...
**Q5 (원리).** 왜 Lambda는 스테이트리스 설계를 강제할까요?

<details><summary>정답 보기</summary>

**수평 확장과 결함 격리를 투명하게 유지하기 위해서입니다.** Lambda는 수요에 따라 수백 개의 실행 환경을 동시에 생성·소멸합니다. 인스턴스별 로컬 상태가 있으면 어떤 인스턴스가 요청을 받느냐에 따라 결과가 달라지고, 특정 인스턴스가 소멸될 때 상태도 함께 사라집니다. 영속 상태는 DynamoDB·S3 같은 외부 저장소로 분리해야 오토스케일링과 재시도가 의도대로 작동합니다.
</details>
