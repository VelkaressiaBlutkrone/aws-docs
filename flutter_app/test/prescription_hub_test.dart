...
void main() {
  ...
  testWidgets('잠김 시 capacity 사유를 응시 횟수보다 우선 표시', (tester) async {
    await tester.pumpWidget(
      _host(
        const PrescriptionHub(
          onReview: _noop,
          ...
          weightedLockedNote: '검증 문항 준비 중',
        ),
      ),
    );
    expect(find.text('약점 집중 모의고사 · 검증 문항 준비 중'), findsOneWidget);
    expect(find.text('약점 집중 모의고사 · 응시 3/3'), findsNothing);
  });
