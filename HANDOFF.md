...
- **게이트 플래그(사람 판단 필요, 문서 순):**
  1. soa-t2-1 §2 블록 "비례 응답" — 타깃 추적 내부 알고리즘 특성의 비공식 표현(메커니즘 함의 수준).
  2. soa-t2-3 §3 블록 "참조 구조" — EBS 스냅샷 블록 관리 서술(공식 동작과 근접, 용어만 비공식).
  3. soa-t2-4 §4 블록 "버전 ID를 키로 변경 이벤트 추적" — 인과 방향 미세 부정확(버전 ID는 결과물).
  4. soa-t4-2 §3 블록 "침해하면 반드시 흔적이 남습니다" — "반드시" 단정어(데이터 소스 범위 내 논리).
  5. soa-t4-3 Q5 "AWS-managed 런북 재사용" — Config/SSM 공식 동작이나 원문 확인 권장.
