...
_갱신: 2026-06-15 · 다음 작업자(사람 또는 새 세션)가 이 문서만 읽고 이어받을 수 있도록._

🔗 라이브: https://velkaressiablutkrone.github.io/aws-docs/

---

## 0. 다음: develop→main 릴리스 + 사용자 우선순위 결정

**브랜치 전략 전환(2026-06-15):** `main` 보호, **`develop` 통합 브랜치 신설**. 작업 브랜치 → develop PR → (릴리스 시) develop → main PR. 전역 규칙 `~/.claude/rules/git-branch-flow.md` 준수.

**대기 1 — 릴리스:** CLF 문항 ≥15(§0-q, 아래) 등 develop 누적분이 **아직 main 미반영(라이브 미배포)**. `develop → main` PR로 릴리스하면 GitHub Pages 자동 배포.

**대기 2 — 다음 작업 우선순위(사용자 결정):**
- **§2 백로그 ②SAA-C03 문항 착수**(현재 0) / **③C-중량 딥링크** — ①CLF ≥15는 완료(§0-q).
...
## 0-q. 완료: CLF 문항 밀도 ≥15 심화 — develop 반영됨 (2026-06-15, main 릴리스 대기)
...
- **① ≥15 심화:** ~~CLF 각 Task 12→15 verified(+57문항).~~ **완료(2026-06-15, §0-q) — develop 반영, main 릴리스 대기.** 워크플로 메모리 [[content-density-loop]].
