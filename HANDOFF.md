...
## 0. 다음: 학습문서 고도화 배치 C(SOA 20) — 마지막 배치

**배치 B(SAA 24) 출고 완료(2026-06-12):** 검수 완료 확인 → 24개 lastVerified=2026-06-12(+출처 줄 병기) → main 병합(`--no-ff`, 681b193) → 머지본 391 테스트 그린 → push(=배포) → 브랜치 삭제(로컬·origin). 검수 피드백 8건(High 2·Med 4·Low 1·잔존 1)은 전부 인용 공식 문서를 WebFetch 대조 후 반영 — 핵심: 스팟 '경매' 모델 교정(기존 본문 포함)·TGW 중복 CIDR 미지원 명시·T 패밀리 standard/unlimited 구분·SR-IOV DMA 제거·LSI 보수화·CUR backfill(12개월) 반영, 공식 출처 7건 추가.

- **배치 C 절차(Plan 2 Task 7~9):** `git checkout -b feat/content-enrichment-soa` → 구조 테스트 `enriched`에 soa-t1-1~t5-4 20개 추가 → **사전 점검 필수: SOA는 CLI 코드 블록 보유 — 코드 펜스 안 `### ` 카운트 오염 확인, 오염 시 펜스-제외 카운트로 테스트 보강(소커밋) 후 RED(20×3=60)** → 문서별 루프 → 전체 그린(391+60=451) → 게이트 STOP(검수 → lastVerified → 병합·배포).
- **SOA 특이사항:** CLI 코드 예시가 있는 서브섹션의 원리 블록은 명령 문법이 아니라 **동작 원리**(경보 상태 전이·지표 집계 등)에 집중.
- **루프(배치 B에서 검증·개선된 형태):** 문서별 ①구현(sonnet) → ②통합 spec+quality 리뷰+수정 권한(sonnet) → ③컨트롤러 tail 직접 실측 + fix 디프 스팟 체크(포인터↔블록 1:1 대조).
- **검수-존치 플래그(배치 B 검수에서 미지적 — 필요 시 재론):** t1-2 '권한 세트' 행 · t3-6 RCU/WCU 행 · t3-8 '비전이' 행 · t3-9 Kinesis 설계 의도 · t4-4 '캐시 히트율' 행.
...
- **테스트 카운팅(main 반영됨, faa16d9):** `^### `만 카운트(#### 제외) + 하한 캡 8. 배치 C에서 코드 펜스 오염 발견 시 펜스-제외 로직 보강.
...
**다음 세션 첫 수: §0 배치 C(SOA 20) 착수. 그 외 대기: §0-b 선택 항목(2기기 pull·개인정보 고지).**
